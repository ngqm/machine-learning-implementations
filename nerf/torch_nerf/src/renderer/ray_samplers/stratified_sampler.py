...
class StratifiedSampler(RaySamplerBase):
    ...
    def sample_along_rays_uniform(
        ...
        t_bins = self.create_t_bins(num_sample+1, device=ray_bundle.origins.device)
        rand = torch.rand(ray_bundle.origins.shape[0], num_sample, device=ray_bundle.origins.device)
        t_samples = t_bins[:-1].unsqueeze(0) + (t_bins[1:] - t_bins[:-1]).unsqueeze(0) * rand
