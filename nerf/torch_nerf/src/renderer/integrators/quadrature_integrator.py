...
class QuadratureIntegrator(IntegratorBase):
    ...
    def integrate_along_rays(
        ...
        """
        ones = torch.ones(sigma.shape[0], 1, device=sigma.device)
        tramsmittance = torch.cat((ones, torch.exp(-torch.cumsum(sigma * delta, dim=1)[:, :-1])), dim=1)
        weights = (1 - torch.exp(-sigma * delta)) * tramsmittance
