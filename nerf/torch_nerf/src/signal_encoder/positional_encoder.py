...
class PositionalEncoder(SignalEncoderBase):
    ...
    def _create_embedding_fn(self):
        ...
        """
        fns = [lambda x: x] if self._include_input else []
        for i in range(self._embed_level):
            fns.append(lambda x, level=i: torch.sin(2**(level) * x))
            fns.append(lambda x, level=i: torch.cos(2**(level) * x))
        return fns
